...
export function useUserProfile() {
    ...
    const initializeUser = async (username, bio, profilePictureUrl) => {
        if (program && publicKey) {
            try {
                ...
                const [profilePda, _] = findProgramAddressSync([utf8.encode('USER_STATE'), publicKey.toBuffer()], program.programId)
                await program.rpc.initialize(username, bio, profilePictureUrl, {
                    accounts: {
    ...
    return { initialized, initializeUser,comments, userComments, combinedPosts, bio,
