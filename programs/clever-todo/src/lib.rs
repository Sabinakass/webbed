...
declare_id!("JAjeGoUouP89ASbY2UsusiFQr8oh14YnVjLPEmQ2gvWH");
...
pub mod web3_linkedin {
    ...
    pub fn send_friend_request(ctx: Context<SendFriendRequest>) -> Result<()> {
        ...
        friend_profile.pending_requests_received.push(sender_profile.key());
        sender_profile.pending_requests_sent.push(friend_profile.key());
...
pub struct WriteComment<'info> {
    ...
    #[account(
        init,
        seeds = [COMMENT_TAG, user_profile.key().as_ref(), post.key().as_ref(), &[post.last_comment as u8].as_ref()],
        bump,
        payer = authority,
        space = 8 + std::mem::size_of::<Comment>(),
    )]
